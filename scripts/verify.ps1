...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tidal-lang-type-detail.ps1

julia --project=. tests/domain_review.jl

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tidal-lang-type-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tidal-lang-type-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-tidal_lang_type_dock-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
