...
`tidal-lang-type-dock` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for type workflows, centered on format conversion, round-trip fixtures, and lossless normalization checks.
...
The score is deterministic. The current threshold is 173. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
